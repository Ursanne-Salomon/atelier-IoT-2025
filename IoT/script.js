...
function afficherMesures() {
  ...
  const pageData = allMesures.slice(debut, fin);
  ...
  tbody.innerHTML = pageData.map(m => `
    <tr>
      ...
      <td>${m.temperature.toFixed(2)}°C</td>
      <td>${m.humidity.toFixed(2)}%</td>
    </tr>
  `).join('');

  afficherGraphique(pageData);
}
...
fetchMesures();
setInterval(fetchMesures, 60000); // Refresh toutes les 60 sec
